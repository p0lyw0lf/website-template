...
* The following properties are provided for a markdown template, under the
...
* `run_js(file)`: runs the javascript `file` relative to the project. If a
...
* `run_tera(template)`: runs the tera template `template` relative to the
